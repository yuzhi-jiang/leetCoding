...
public class MySortUtil2 {
    ...
    public static void insertSort(int[] arrs) {
        ...
        int cur;
        int j;
        for (int i = 1; i < n; i++) {//无序中取出第一个 i>0
            cur = arrs[i];
            for (j = i - 1; j >= 0; j--) {//在无序的前面  0....i-1 中找到合适的位置
                if (cur > arrs[j]) break;
            }
            if (j != i - 1) {//在j+1初插入
                //找到了合适的位置，其他的往后移动
                for (int t = i - 1; t > j; t--) {   // t t-1 t-2 ...j+2 j+1 j  ->>> j到i-1之间的数后移一位
                    arrs[t + 1] = arrs[t];
                }
                arrs[j + 1] = cur;//j的后面空出一个，放入
            }
        }
    }
    ...
    public static void bucketSort(int[] a, int max) {
        if (a == null || a.length < 2 || max < 2) return;
        int[] bucket = new int[max];//开辟桶
    ...
    public static void quickSort(int[] arr) {
        quickSortByLAndR(arr, 0, arr.length - 1);
    }

    private static void quickSortByLAndR(int[] a, int l, int r) {
        if (l < r) {
            int i, j, x;
            i = l;
            x = a[i];
            ...
            a[i] = x;
            quickSortByLAndR(a, l, i - 1);
            quickSortByLAndR(a, i + 1, r);
        }
    }
}
